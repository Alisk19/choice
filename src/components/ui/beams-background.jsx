...
export function BeamsBackground({ className = "", intensity = "strong", children }) {
    ...
    return (
        ...
        >
            {/* Canvas MUST be z-index 0, positioned absolute, pointer-events none */}
            <canvas
                ref={canvasRef}
                style={{
                    position: 'absolute',
            ...
            {/* Content MUST be z-index 10 so it renders ABOVE the canvas */}
            <div className={className} style={{ position: 'relative', zIndex: 10, height: '100%', width: '100%' }}>
                {children}
            </div>
        </div>
    );
